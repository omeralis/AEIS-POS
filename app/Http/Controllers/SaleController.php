    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['biller'] = isset($sale->biller) ? $sbiller = $sale->biller->name:$sbiller="";
                $nestedData['customer'] = isset($sale->customer) ? $sale->customer->name :"";
                ...
                $nestedData['sale'] = array( '[ "'.date(config('date_format'), strtotime($sale->created_at->toDateString())).'"', ' "'.$sale->reference_no.'"', ' "'.$sale_status.'"', ' "'.optional($sale->biller)->name.'"', ' "'.optional($sale->biller)->company_name.'"', ' "'.optional($sale->biller)->email.'"', ' "'.optional($sale->biller)->phone_number.'"', ' "'.optional($sale->biller)->address.'"', ' "'.optional($sale->biller)->city.'"', ' "'.optional($sale->customer)->name.'"', ' "'.optional($sale->customer)->phone_number.'"', ' "'.optional($sale->customer)->address.'"', ' "'.optional($sale->customer)->city.'"', ' "'.$sale->id.'"', ' "'.$sale->total_tax.'"', ' "'.$sale->total_discount.'"', ' "'.$sale->total_price.'"', ' "'.$sale->order_tax.'"', ' "'.$sale->order_tax_rate.'"', ' "'.$sale->order_discount.'"', ' "'.$sale->shipping_cost.'"', ' "'.$sale->grand_total.'"', ' "'.$sale->paid_amount.'"', ' "'.$sale->sale_note.'"', ' "'.$sale->staff_note.'"', ' "'.$sale->user->name.'"', ' "'.$sale->user->email.'"', ' "'.$sale->warehouse->name.'"', ' "'.$coupon_code.'"', ' "'.$sale->coupon_discount.'"]'
                );
                $data[] = $nestedData;
            }
        }
